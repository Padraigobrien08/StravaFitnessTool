...
export function ecosystemInsightsToCoachInsights(
  ...
  return ecosystem.ecosystemInsights.map((e) => ({
    ...
    title: e.title,
    severity: e.severity,
    evidence: [...e.evidence, `Confidence: ${e.confidence}. ${e.limitations[0] ?? ""}`].filter(
      Boolean,
    ),
...
export function ecosystemHeadline(ecosystem: TrainingEcosystemAnalysis | null): string | null {
  if (!ecosystem) return null;
  const hi = ecosystem.interferenceFlags.filter(
    (f) => f.severity !== "low" && f.kind === "near_quality_run",
  );
  if (hi.length > 0) {
    const s = ecosystem.scores.strengthSupport >= 60 ? "Strength support is consistent, but " : "";
    return `${s}${hi[0].message}`;
  }
  const insight = ecosystem.ecosystemInsights.find((i) => i.severity === "positive");
...
export function generateEcosystemInsights(ecosystem: TrainingEcosystemAnalysis | null): Insight[] {
