...
function pass(ruleId: string, message: string, evidence?: string[]): ValidationRuleResult {
  return {
    ruleId,
...
export function runRecommendationRules(
  ...
  const recClaimsHigh = rec.includes("medium-high") || rec.includes("high confidence");
  ...
  const volumeUp = /increase volume|add volume|build (mileage|volume)|more weekly volume/.test(rec);
  ...
  const strengthInterfering =
    ...
    forecast.limitations.some((l) => /strength.*(interfer|timing)/i.test(l.detail));
  ...
  const aggressiveWhileFatigued =
    fatigued && /add (a |another )?hard|stack|intensity block|threshold session/.test(rec);
