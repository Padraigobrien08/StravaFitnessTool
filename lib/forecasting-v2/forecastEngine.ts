...
import { buildCapabilityModelEstimates, computeWeightedCapability } from "./capabilityModels";
...
import { buildPredictionInterval, buildUncertaintyAssessment } from "./uncertaintyModel";
...
function modelAgreement(
  ...
  const mean = estimates.reduce((s, e) => s + e.predictedTimeSec, 0) / estimates.length;
...
function nearRaceEffortCeilingSec(
  ...
  const durabilityTax = input.goal.distanceMeters >= 21000 ? 1.015 : 1.01;
  return Math.round(extrapolated * durabilityTax + Math.max(0, freshnessAdjustmentSec));
}
...
function buildRecommendation(
  ...
  parts.push(`Limitation: ${limitations[0] ?? "No sleep or HRV data available."}`);
...
export function buildRaceForecastV2(input: RaceForecastInput): RaceForecastV2 {
  ...
  const { baseTimeSec, weightedEstimates, spreadSec } = computeWeightedCapability(input, estimates);
  ...
  const modelTimes = weightedEstimates.map((e) => e.predictedTimeSec).filter((t) => t > 0);
  ...
  const nearRaceCap = nearRaceEffortCeilingSec(input, freshness.timeAdjustmentSec);
