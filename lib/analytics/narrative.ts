...
export function buildWeeklyNarrative(
  ...
  const { current, previous } = buildCurrentAndPreviousWeek(
    runs,
    input.athleteMaxHr,
    weekOffset
  );
  ...
  const target =
    input.goalProgress?.targetPerWeek ?? defaultWeeklyRuns;
  ...
  if (current.runCount === 0) {
    paragraphs.push(
      `No runs recorded for the week of ${current.weekLabel}.`
    );
    bullets.push("0 runs this week");
  } else if (current.runCount >= target) {
    const met =
      current.runCount === target
        ? "matching"
        : "exceeding";
    paragraphs.push(
    ...
    bullets.push(
      `${current.runCount} / ${target} runs this week (target met)`
    );
  } else {
    ...
    bullets.push(
      `${current.runCount} / ${target} runs this week`
    );
  }
  ...
  if (previous && previous.runCount > 0) {
    ...
    bullets.push(
      `Volume: ${current.distanceKm} km vs ${previous.distanceKm} km last week`
    );
  ...
  if (current.longestRunKm > 0) {
    ...
    const priorMax = maxLongestRunPriorWeeks(
      runs,
      input.athleteMaxHr,
      beforeStart,
      4
  ...
  if (current.runCount === 0) severity = "warning";
    ...
    current.runCount >= target &&
    input.efficiencySummary.trend === "improving"
  ) {
    severity = "positive";
