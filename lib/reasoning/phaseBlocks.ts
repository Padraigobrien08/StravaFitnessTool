...
export function buildPhaseBlocks(ctx: ReasoningContext, maxBlocks = 12): PhaseBlockMetrics[] {
  const runs = ctx.runs;
  ...
  for (let w = 0; w < maxBlocks; w++) {
    ...
    const distanceKm = Math.round(inBlock.reduce((s, r) => s + r.distanceM / 1000, 0) * 10) / 10;
    const longestRunKm = Math.round(Math.max(...inBlock.map((r) => r.distanceM)) / 100) / 10;
    const longRunPctOfVolume =
      distanceKm > 0 ? Math.round((longestRunKm / distanceKm) * 1000) / 10 : 0;
    ...
    const hardPct = inBlock.length > 0 ? Math.round((hardCount / inBlock.length) * 1000) / 10 : 0;
    ...
    const weeklyKmVariance =
      kmValues.length >= 2
        ? Math.round((Math.max(...kmValues) - Math.min(...kmValues)) * 10) / 10
        : 0;

    const hardBandScore = hardPct >= 15 && hardPct <= 25 ? 1 : hardPct < 15 ? 0.6 : 0.4;
    const effScore = meanEfficiency != null ? Math.max(0, 1 - meanEfficiency * 100) : 0.5;
    const aerobicScore = Math.round((hardBandScore * 0.4 + effScore * 0.6) * 100);
