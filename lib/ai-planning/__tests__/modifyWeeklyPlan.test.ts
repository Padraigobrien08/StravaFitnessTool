...
describe("modifyWeeklyPlan follow-ups", () => {
  ...
  it("reduces volume", () => {
    ...
    const plan = buildSafeFallbackWeeklyPlan(ctx, g);
    const before =
      plan.totalRunDistanceKm ??
      plan.workouts.reduce((s, w) => s + (w.distanceKm ?? 0), 0);
    const out = applyPlanModification(plan, "reduce_volume", g);
    const after =
      out.totalRunDistanceKm ??
      out.workouts.reduce((s, w) => s + (w.distanceKm ?? 0), 0);
  ...
  it("limits to available days", () => {
    ...
    const days = out.workouts
      .filter((w) => w.modality !== "rest")
