...
export function serializeAdaptiveIntelligenceForCoach(
      ...
      sections.push(
        "## Adaptation signals\n" +
          snap.adaptationSignals
            ...
            .map(
              (s) =>
                `- ${s.statement} (${s.confidence}, ${s.stability})`
            )
            .join("\n")
      );
    }
  ...
  if (!topic || topic === "all" || topic === "outcomes") {
    const outcomes = snap.recommendationOutcomes.filter(
      (o) => o.evaluatedAt
    );
    if (outcomes.length) {
      sections.push(
        "## Recommendation outcomes\n" +
          outcomes
            ...
            .map(
              (o) =>
                `- [${o.evaluation}] ${o.recommendation.slice(0, 100)}`
            )
            .join("\n")
      );
    }
