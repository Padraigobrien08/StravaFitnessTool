...
export function AdaptationTrendsPanel({
  trends,
}: {
  trends: AdaptationTrendView[];
}) {
  return (
    <PanelChrome title="Adaptation trends" href="/trends">
      <p className={cn(dash.muted, "mb-4 max-w-2xl")}>
        Signals that explain whether you are getting faster — charts support the
        ...
        {trends.map((t) => (
          ...
          >
            <div className="flex items-baseline justify-between gap-2">
              ...
              {t.caption ? (
                <span className="text-[10px] tabular-nums text-teal-400/75">
                  {t.caption}
                </span>
              ) : null}
            </div>
