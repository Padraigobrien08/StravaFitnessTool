...
function typography(defaultTag: React.ElementType, baseClass: string) {
  return function Typography({ as, className, children, ...props }: TypographyProps) {
...
export function TypographyList({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      className={cn(
        "my-3 ml-4 list-disc space-y-1.5 text-[0.9375rem] leading-[1.55] text-muted-foreground [&>li]:mt-0",
        className,
      )}
      {...props}
    />
