...
**Status:** P1–P4 shipped; outcome metric refined with execution grade (`feat/leg-feel-exec-grade`) · **Remaining:** HR-drift / skipped-session signals + weight validation on production data · **Risk:** low–medium
...
**Shipped in P4 (`lib/wellness/outcomeCalibration.ts`):** true feel↔**outcome** recalibration. It pairs each directional report with the athlete's running in the next 0–2 days (aerobic efficiency vs. their baseline — _did they actually run worse when they said heavy?_) and, unlike P3, is **bidirectional**: predictive reporters are trusted more, counter-predictive ones less. It's Laplace-shrunk toward the default on small samples and floored so a "heavy" report never drops readiness by less than 6 (safety). The outcome signal is itself a ladder — **session execution grade** (`scoreSessionExecution`, the most direct "did the session go well?" signal, used when a run has FIT streams) → aerobic efficiency vs. baseline → and the whole calibration then falls back to the P3 agreement proxy → flat default as evidence thins. **Remaining:** add HR-drift / skipped-session signals and validate the weights on real usage data.
