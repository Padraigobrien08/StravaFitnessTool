...
**Remaining:** C4 (low urgency) and the rest of **F6** (monthly/pre-race narratives, phase-catalog UI). Plus fast-follows: Home "Today" card + MCP parity (F1), race-strategy split charts in miles (F2), `executeIntelligenceTool` integration tests (C3), fold real recommendation outcomes into persisted beliefs + coach-memory route persistence, training-monotony/strain risk pattern.
