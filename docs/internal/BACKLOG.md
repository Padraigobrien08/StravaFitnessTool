...
| F6     | ~~Risk-pattern matching~~ · ~~monthly/pre-race narratives~~ · phase-catalog UI · ~~recommendation-outcome tracking~~ · ~~persisted AthleteMemory~~ | M–L each | Med      | Longer-horizon differentiation depth. Done: recommendation-outcome tracking (#45–#47), persisted AthleteMemory (#48), risk-pattern matching (#49), **monthly/pre-race narratives** (#50): `buildMonthlyNarrative` + `buildPreRaceNarrative` + `get_monthly_narrative`/`get_pre_race_narrative` tools + Intelligence panel. **Only phase-catalog UI remains.** |
