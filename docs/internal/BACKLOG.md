...
| ~~C4~~ | ✅ Tighten `as unknown as` casts + `plan-workspace` `runs` re-alloc — **done** (#52) | M    | Widened `StravaStreamSeries.data` to `number[] \| [number,number][]` (latlng honest) → double-casts became single narrowing casts; `PersistableWeek` removes the training-calendar route cast; stable `EMPTY_RUNS` fixes the memo re-alloc. Remaining `as unknown as` in `client.ts` are legitimate driver-adapter bridges. |
...
| ~~F6~~ | ✅ Risk-pattern matching · monthly/pre-race narratives · phase-catalog UI · recommendation-outcome tracking · persisted AthleteMemory — **all done** | M–L each | Med      | Longer-horizon differentiation depth — **fully shipped**: recommendation-outcome tracking (#45–#47), persisted AthleteMemory (#48), risk-pattern matching (#49), monthly/pre-race narratives (#50), phase-catalog UI (#51): `lib/analytics/trainingPhases.ts` (week classifier → merged typed phases) + `get_training_phases` tool + `/training` band-timeline panel |
...
- ✅ **F6 (risk-pattern matching)** — named risk flags w/ evidence + mitigation (#49)
- ✅ **F6 (monthly/pre-race narratives)** — period narratives + tools + panel (#50)
- ✅ **F6 (phase-catalog UI)** — training-phase timeline + tool + panel (#51). **F6 complete.**

**Remaining:** only **C4** (low urgency) plus fast-follows — the F1–F6 feature roadmap is fully shipped. Fast-follows: Home "Today" card + MCP parity (F1), race-strategy split charts in miles (F2), `executeIntelligenceTool` integration tests (C3), fold real recommendation outcomes into persisted beliefs + coach-memory route persistence, training-monotony/strain risk pattern, monthly narrative in the printable report.
