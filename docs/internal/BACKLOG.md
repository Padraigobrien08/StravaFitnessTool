...
| F6     | Risk-pattern matching · monthly/pre-race narratives · phase-catalog UI · ~~recommendation-outcome tracking~~ · persisted AthleteMemory | M–L each | Med      | Longer-horizon differentiation depth. **Recommendation-outcome tracking done** (#45): adherence log + `get_recommendation_outcomes`. Fast-follows: outcome-signal integration, Intelligence panel, log week-plan/goal-scenario producers |
