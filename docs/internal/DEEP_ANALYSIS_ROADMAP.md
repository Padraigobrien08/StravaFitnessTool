...
| G2     | Sensitivity / tornado chart                             | M    | Local partials: which input moves the forecast most (+5 km long run → −Xs; +10 freshness → −Ys). Generalize `goalScenarios` perturbation into a live what-if sandbox with sliders.                                     |
| G3     | Ensemble transparency                                   | S–M  | Surface each model (Riegel, Cameron, power-law, critical-speed) estimate + weight + why-weighted, plus model agreement/spread as a confidence signal.                                                                  |
| G4     | Confidence decomposition                                | S    | Turn the uncertainty band into "what's widening it" with each driver's contribution (few efforts, stale data, high variance).                                                                                          |
| G5     | **Self-auditing calibration**                           | M–L  | Log every forecast; when a real effort lands, score it. Calibration plot: "when I say 70%, does it happen 70%?" A forecaster that grades itself and tightens over time.                                                |
...
| #   | Item                            | Size | Notes                                                                                                                                                                              |
| --- | ------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | Critical Speed (CS) + D′        | M    | Fit the speed–duration curve across best efforts → aerobic ceiling (CS) vs anaerobic reserve (D′), two separate trainable numbers. Add as a capability model in the ensemble (G3). |
| P2  | Personalized fatigue-resistance | S    | Surface the fitted power-law exponent ("your Riegel 1.09 vs elite ~1.06 → you fade more over distance") + trend.                                                                   |
| P3  | Durability score                | M    | Decoupling (`workoutQuality`) + last-quarter long-run pace → fatigue-resistance metric + trend. The modern differentiator.                                                         |
